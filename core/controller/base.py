...
class BaseController(Generic[ModelType]):
    ...
    async def get_by_uuid(self, uuid: str, join_: set[str] | None = None) -> ModelType:
        """
        ...
        db_obj = await self.repository.get_by_field(
            field="uuid", value=uuid, joins=join_
        )
        if not db_obj:
    ...
    async def get_all(
        ...
        response = await self.repository.get_all(skip, limit, joins=join_)
        return response
