...
\begin{tikzpicture}[
    scale=1.0,
    node distance=3.5cm,
    actor/.style={rectangle, draw, fill=blue!10, minimum width=2.5cm, minimum height=0.8cm, font=\small\bfseries},
    message/.style={->, >=stealth, thick},
    label/.style={font=\footnotesize, fill=white, inner sep=2pt}
]

% Actors (proper positioning with consistent spacing)
\node[actor] (browser) {Browser};
\node[actor, right=3.5cm of browser] (api) {Recording API};
\node[actor, right=3.5cm of api] (domain) {Domain Layer};
\node[actor, right=3.5cm of domain] (minio) {MinIO};

% Lifelines (longer for better readability)
...
% Messages with proper spacing (1cm between each)
% Self-call for browser recording
\draw[message] ([yshift=-0.8cm]browser.south) -- node[label, right] {1. Record chunk} ++(1.5cm,0) -- ++(0,-0.4cm) -- ++(-1.5cm,0);

% Browser to API
\draw[message] ([yshift=-1.8cm]browser.south) -- node[label, above] {2. POST /uploads/chunk} ([yshift=-1.8cm]api.south);

% API to Domain
\draw[message] ([yshift=-2.8cm]api.south) -- node[label, above] {3. validate\_checksum()} ([yshift=-2.8cm]domain.south);

% Domain to MinIO
\draw[message] ([yshift=-3.8cm]domain.south) -- node[label, above] {4. upload\_chunk()} ([yshift=-3.8cm]minio.south);

% MinIO back to Domain
\draw[message] ([yshift=-4.8cm]minio.south) -- node[label, below] {5. 200 OK} ([yshift=-4.8cm]domain.south);

% Self-call for domain event
\draw[message] ([yshift=-5.6cm]domain.south) -- node[label, right] {6. Emit event} ++(1.2cm,0) -- ++(0,-0.3cm) -- ++(-1.2cm,0);

% Domain back to API
\draw[message] ([yshift=-6.6cm]domain.south) -- node[label, below] {7. Response} ([yshift=-6.6cm]api.south);

% API back to Browser
\draw[message] ([yshift=-7.6cm]api.south) -- node[label, below] {8. \{chunk\_id, progress\}} ([yshift=-7.6cm]browser.south);

% Repeat note at bottom
\node[font=\footnotesize\bfseries, text width=10cm, align=center] at (6,-9.5) {9. Repeat every 5 seconds during recording...};
