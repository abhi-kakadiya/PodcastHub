...
\begin{tikzpicture}[
    node distance=0.3cm,
    actor/.style={rectangle, draw, minimum width=1.5cm, minimum height=0.6cm, font=\footnotesize},
    message/.style={->, thick}
]

% Actors
\node[actor] (browser) {Browser};
\node[actor, right=1.5cm of browser] (api) {API};
\node[actor, right=1.5cm of api] (domain) {Domain};
\node[actor, right=1.5cm of domain] (minio) {MinIO};

% Lifelines
\draw[dashed] (browser.south) -- ++(0,-8cm);
\draw[dashed] (api.south) -- ++(0,-8cm);
\draw[dashed] (domain.south) -- ++(0,-8cm);
\draw[dashed] (minio.south) -- ++(0,-8cm);

% Messages
\draw[message] ($(browser.south)+(0,-0.5)$) -- node[above, font=\tiny] {1. MediaRecorder.ondataavailable} ($(browser.south)+(0,-1)$);

\draw[message] ($(browser.south)+(0,-1.5)$) -- node[above, font=\tiny] {2. POST /uploads/chunk} ($(api.south)+(0,-1.5)$);

\draw[message] ($(api.south)+(0,-2)$) -- node[above, font=\tiny] {3. validate\_checksum()} ($(domain.south)+(0,-2)$);

\draw[message] ($(domain.south)+(0,-2.5)$) -- node[above, font=\tiny] {4. upload\_chunk()} ($(minio.south)+(0,-2.5)$);

\draw[message] ($(minio.south)+(0,-3)$) -- node[above, font=\tiny] {5. 200 OK} ($(domain.south)+(0,-3)$);

\draw[message] ($(domain.south)+(0,-3.5)$) -- node[above, font=\tiny] {6. ChunkUploaded event} ($(domain.south)+(0,-4)$);

\draw[message] ($(domain.south)+(0,-4.5)$) -- node[above, font=\tiny] {7. response} ($(api.south)+(0,-4.5)$);

\draw[message] ($(api.south)+(0,-5)$) -- node[above, font=\tiny] {8. \{chunk\_id, progress\}} ($(browser.south)+(0,-5)$);

\node[right=0.1cm of $(browser.south)+(0,-6)$, font=\tiny] {9. Continue every 5s...};
